...
NSString * BinneyTimothy_Week3$ModuleRequireFormat = @"(function(exports){"
...
@implementation BinneyTimothy_Week3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_binneytimothy_week3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create BinneyTimothy_Week3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "BinneyTimothy_Week3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_binneytimothy_week3 = [[BinneyTimothy_Week3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_binneytimothy_week3];
	
	NSString *_binneytimothy_week3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _binneytimothy_week3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_binneytimothy_week3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:BinneyTimothy_Week3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.binneytimothy_week3.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.binneytimothy_week3.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
